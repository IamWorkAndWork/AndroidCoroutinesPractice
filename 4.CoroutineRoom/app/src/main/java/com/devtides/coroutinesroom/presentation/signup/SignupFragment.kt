...
class SignupFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        goToUserListBtn.setOnClickListener { onGoToUserList(it) }
    ...
    private fun onGotoLogin(v: View?) {
        ...
        v?.let { Navigation.findNavController(it).navigate(action) }
    }

    private fun onGoToUserList(v: View?) {
        val action =
            SignupFragmentDirections.actionSignupFragmentToUserListFragment()
        v?.let { Navigation.findNavController(it).navigate(action) }
    }
}
